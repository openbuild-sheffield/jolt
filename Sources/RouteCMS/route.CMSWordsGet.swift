...
public class RequestCMSWordsGet: OpenbuildExtensionPerfect.RequestProtocol {

    public var method: String
    public var uri: String
    public var description: String = "Get a CMS words entity by handle."
...
public let handlerRouteCMSWordsGet = { (request: HTTPRequest, response: HTTPResponse) in
    ...
    do {
        ...
        if entity == nil {
            ...
            handlerResponse.complete(
                status: 200,
                model: ModelCMSWords200Entity(entity: entity!)
            )
...
public class RouteCMSWordsGet: OpenbuildRouteRegistry.FactoryRoute {

    override public func route() -> NamedRoute? {

        let handlerResponse = ResponseDefined()
        handlerResponse.register(status: 200, model: "RouteCMS.ModelCMSWords200Entity")
        handlerResponse.register(status: 404)
        handlerResponse.register(status: 500)

        return NamedRoute(
            handlerRequest: RequestCMSWordsGet(
                method: "get",
                uri: "/api/cms/words/{handle}"
            ),
            handlerResponse: handlerResponse,
            handlerRoute: handlerRouteCMSWordsGet
        )
