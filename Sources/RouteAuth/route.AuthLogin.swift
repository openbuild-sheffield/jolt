...
public class RequestAuthLogin: OpenbuildExtensionPerfect.RequestProtocol {

    public var method: String
    public var uri: String
    public var description: String = "Allows the user to login to the system."
...
public let handlerRouteAuthLogin = { (request: HTTPRequest, response: HTTPResponse) in
    ...
    do{
        ...
        guard let token = repository.createToken(model: user) else {

            handlerResponse.complete(
                status: 500,
                model: ResponseModel500Messages(messages: [
                    "token": "Failed to create the token."
                ])
            )
        ...
        handlerResponse.complete(
            status: 200,
            model: ModelToken(token: token)
        )

    } catch OpenbuildRepository.RepoError.connection(let messagePublic, let messageDev) {
        ...
        handlerResponse.complete(
            status: 500,
            model: ResponseModel500Messages(messages: [
                "repository_connection": messagePublic
            ])
        )
...
public class RouteAuthLogin: OpenbuildRouteRegistry.FactoryRoute {

    override public func route() -> NamedRoute? {

        let handlerResponse = ResponseDefined()
        handlerResponse.register(status: 200, model: "RouteAuth.ModelToken")
        handlerResponse.register(status: 422)
        handlerResponse.register(status: 500)

        return NamedRoute(
            handlerRequest: RequestAuthLogin(
                method: "post",
                uri: "/api/auth/login"
            ),
            handlerResponse: handlerResponse,
            handlerRoute: handlerRouteAuthLogin
        )
    }
